...
export const CountDown = ({ waktu_pengerjaan, onCountdownZero }: CountdownProps) => {
    ...
    useEffect(() => {
        if (countdown['w-verbal'] <= 20 && startCountdown) {
            setColor(true);
        }
        if (countdown['w-verbal'] <= 0 && startCountdown) {
                ...
                <div>
                    ...
                    {startCountdown ? (
                        <h1 className={cn("text-6xl", color ? "text-red-500" : "")}>
                            {formatTimeCountdown(countdown['w-verbal'])}
                        </h1>
                    ) : (
