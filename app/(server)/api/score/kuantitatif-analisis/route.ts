...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalKuantitatifAnalisis.findUnique({
    ...
    try {
        const existingScoreTotal = await prisma.scoreTotalPsikotest.findUnique({
            ...
            select: {
                ...
                id_score_kuantitatif_analisis: true,
            },
        });
        ...
        const existingScoreId = existingScoreTotal?.id_score_kuantitatif_analisis;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreKuantitatifAnalisis.create({
                data: {
                    benar: correctAnswer,
                salah: wrongAnswer,
                score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes kuantitatif analisis anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreKuantitatifAnalisis.update({
            where: {
                id: existingScoreId,
            },
            data: {
