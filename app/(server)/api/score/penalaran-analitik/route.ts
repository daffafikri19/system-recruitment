...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalPenalaranAnalitik.findUnique({
    ...
    try {
        const existingScoreTotal = await prisma.scoreTotalPsikotest.findUnique({
            ...
            select: {
                ...
                id_score_penalaran_analitik: true,
            },
        });
        ...
        const existingScoreId = existingScoreTotal?.id_score_penalaran_analitik;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scorePenalaranAnalitik.create({
                data: {
                    benar: correctAnswer,
                salah: wrongAnswer,
                score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes penalaran analitik anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scorePenalaranAnalitik.update({
            where: {
                id: existingScoreId,
            },
            data: {
