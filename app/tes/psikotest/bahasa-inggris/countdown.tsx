...
export const CountDown = ({ waktu_pengerjaan, onCountdownZero }: CountdownProps) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (params.tes === '1' || !params.tes) {
                ...
                setCountdown({
                    'w-bahasa-inggris': waktu_pengerjaan!,
                });
            }
        }
    }, [waktu_pengerjaan]);
    ...
    useEffect(() => {
        if (countdown['w-bahasa-inggris'] <= 20 && startCountdown) {
            setColor(true);
        }
        if (countdown['w-bahasa-inggris'] <= 0 && startCountdown) {
        ...
        if (startCountdown) {
            countdownInterval = setInterval(() => {
                setCountdown((prevTime: any) => {
                    const newTime = {
                        ...prevTime,
                        'w-bahasa-inggris': prevTime['w-bahasa-inggris'] - 1,
                    };
                    return newTime;
                });
            }, 1000);
                ...
                <div>
                    ...
                    {startCountdown ? (
                        <h1 className={cn("text-6xl", color ? "text-red-500" : "")}>
                            {formatTimeCountdown(countdown['w-bahasa-inggris'])}
                        </h1>
                    ) : (
