...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalAntonimSinonim.findUnique({
    ...
    try {
        ...
        const existingScoreId = existingScoreTotal?.id_score_antonim_sinonim;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreAntonimSinonim.create({
                data: {
                    benar: correctAnswer,
                    salah: wrongAnswer,
                    score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes antonim-sinonim anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreAntonimSinonim.update({
            where: {
                id: existingScoreId,
            },
            data: {
