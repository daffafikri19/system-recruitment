...
export const DataTable = ({ questionData }: dataTableProps) => {
    ...
    const handleOpenPratinjau = (data: soalVerbal) => {
        setOpenPratinjau(true);
        setPratinjau(data)
    }

    const [searchQuery, setSearchQuery] = useState("");
    const [filteredData, setFilteredData] = useState<soalVerbal[]>([]);
                            ...
                            <CardContent>
                                {onFile ? (
                                    <div className="relative space-y-3 p-4 rounded-lg border-2 border-dashed w-full flex items-center justify-center">
                                        <Button disabled={loading} className="flex items-center justify-center" variant="success" onClick={handleFileMultipleUpload}>
                                            {loading ? (
                                                <Loader2 className="w-5 h-5 mx-4 animate-spin" />
                                            ): "Import"}
                                        </Button>
                                        <X className="absolute text-red-500 -top-2 right-0 cursor-pointer"
                                         onClick={() => setOnFile(null)} />
                                    </div>
                                ) : (
