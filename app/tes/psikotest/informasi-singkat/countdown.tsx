...
export const CountDown = ({ waktu_pengerjaan, onCountdownZero }: CountdownProps) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (params.tes === '1' || !params.tes) {
                ...
                setCountdown({
                    'w-informasi-singkat': waktu_pengerjaan!,
                });
            }
        }
    }, [waktu_pengerjaan]);
    ...
    useEffect(() => {
        if (countdown['w-informasi-singkat'] <= 20 && startCountdown) {
            setColor(true);
        }
        if (countdown['w-informasi-singkat'] <= 0 && startCountdown) {
        ...
        if (startCountdown) {
            countdownInterval = setInterval(() => {
                setCountdown((prevTime: any) => {
                    const newTime = {
                        ...prevTime,
                        'w-informasi-singkat': prevTime['w-informasi-singkat'] - 1,
                    };
                    return newTime;
                });
            }, 1000);
                ...
                <div>
                    ...
                    {startCountdown ? (
                        <h1 className={cn("text-6xl", color ? "text-red-500" : "")}>
                            {formatTimeCountdown(countdown['w-informasi-singkat'])}
                        </h1>
                    ) : (
