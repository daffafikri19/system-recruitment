...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalKepribadian.findUnique({
    ...
    try {
        ...
        const existingScoreId = existingScoreTotal?.id_score_kepribadian;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreKepribadian.create({
                data: {
                    benar: correctAnswer,
                salah: wrongAnswer,
                score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes kepribadian anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreKepribadian.update({
            where: {
                id: existingScoreId,
            },
            data: {
