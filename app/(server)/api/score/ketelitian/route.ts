...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalKetelitian.findUnique({
    ...
    try {
        ...
        const existingScoreId = existingScoreTotal?.id_score_ketelitian;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreKetelitian.create({
                data: {
                    benar: correctAnswer,
                salah: wrongAnswer,
                score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes ketelitian anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreKetelitian.update({
            where: {
                id: existingScoreId,
            },
            data: {
