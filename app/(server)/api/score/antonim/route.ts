...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalAntonim.findUnique({
    ...
    try {
        ...
        const existingScoreId = existingScoreTotal?.id_score_antonim;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreAntonim.create({
                data: {
                    benar: correctAnswer,
                    salah: wrongAnswer,
                    score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes antonim anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreAntonim.update({
            where: {
                id: existingScoreId,
            },
            data: {
