...
export const CountDown = ({ waktu_pengerjaan, onCountdownZero }: CountdownProps) => {
    ...
    useEffect(() => {
        if (countdown['w-sinonim'] <= 20 && startCountdown) {
            setColor(true);
        }
        if (countdown['w-sinonim'] <= 0 && startCountdown) {
        ...
        if (startCountdown) {
            countdownInterval = setInterval(() => {
                setCountdown((prevTime: any) => {
                    const newTime = {
                        ...prevTime,
                        'w-sinonim': prevTime['w-sinonim'] - 1,
                    };
                    return newTime;
                });
            }, 1000);
                ...
                <div>
                    ...
                    {startCountdown ? (
                        <h1 className={cn("text-6xl", color ? "text-red-500" : "")}>
                            {formatTimeCountdown(countdown['w-sinonim'])}
                        </h1>
                    ) : (
