...
import { addMultipleSoal } from "@/actions/mutations/soal/kalimat-tak-teratur/addMultipleSoal"
import { deleteSoal } from "@/actions/mutations/soal/kalimat-tak-teratur/deleteSoal"
import { useLoadingContext } from "@/lib/providers/loadingStateProvider"
import { Badge } from "@/components/ui/badge"

interface dataTableProps {
    questionData: soalKalimatTakTeratur[]
}
...
export const DataTable = ({ questionData }: dataTableProps) => {
    ...
    const [pratinjau, setPratinjau] = useState<soalKalimatTakTeratur>({
        id: 0,
        gambar: "",
        soal: "",
        kunci_jawaban: "",
    ...
    const handleOpenPratinjau = (data: soalKalimatTakTeratur) => {
        setOpenPratinjau(true);
        setPratinjau(data)
    }

    const [searchQuery, setSearchQuery] = useState("");
    const [filteredData, setFilteredData] = useState<soalKalimatTakTeratur[]>([]);
    ...
    const handleDownloadTemplate = () => {
        const templateURL = "/file/template-soal-kalimat-tak-teratur.xlsx";
    ...
    return (
        <div className="w-full">
            <div className="w-full flex items-center justify-between space-x-8">
                <Button onClick={() => router.push('/dashboard/kelola-soal/kalimat-tak-teratur/add')}>Tambah Soal <Plus className="w-4 h-4 ml-2" /></Button>
                                    ...
                                    <TableCell className="text-center">
                                        <div className="flex justify-center space-x-2">
                                            <Button size="icon" variant="outline" asChild>
                                                <Link href={`/dashboard/kelola-soal/kalimat-tak-teratur/edit/${question.id}/?id=${question.id}`}>
