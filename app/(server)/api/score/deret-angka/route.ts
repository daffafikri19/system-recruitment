...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalDeret.findUnique({
    ...
    try {
        ...
        const existingScoreId = existingScoreTotal?.id_score_deret;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreDeretAngka.create({
                data: {
                    benar: correctAnswer,
                    salah: wrongAnswer,
                    score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes deret angka anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreDeretAngka.update({
            where: {
                id: existingScoreId,
            },
            data: {
