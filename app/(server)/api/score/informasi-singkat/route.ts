...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalInformasiSingkat.findUnique({
    ...
    try {
        const existingScoreTotal = await prisma.scoreTotalPsikotest.findUnique({
            ...
            select: {
                ...
                id_score_informasi_singkat: true,
            },
        });
        ...
        const existingScoreId = existingScoreTotal?.id_score_informasi_singkat;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreInformasiSingkat.create({
                data: {
                    benar: correctAnswer,
                    salah: wrongAnswer,
                    score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes informasi singkat anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreInformasiSingkat.update({
            where: {
                id: existingScoreId,
            },
            data: {
