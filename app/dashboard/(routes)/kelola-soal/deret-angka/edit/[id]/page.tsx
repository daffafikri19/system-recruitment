...
import { editSoal } from '@/actions/mutations/soal/deret-angka/editSoal'
...
const EditSoalDeretAngkaPage = () => {
  const [media, setMedia] = useState<string | null>("");
  const [soal, setSoal] = useState("");
  const router = useRouter();
  const { loading, setLoading } = useLoadingContext();

  const [question, setQuestion] = useState<SoalDeretAngkaProps>({
    id: 0,
    gambar: "",
    soal: "",
    A: "",
        ...
        <CardContent className='py-4'>
          <form action={async formdata => {
            ...
            if (result.status === 200) {
              ...
              setLoading(false)
              return router.push('/dashboard/kelola-soal/deret-angka')
            } else {
              toast({
                title: result.message,
                variant: "destructive"
            ...
            <div className='mt-10 w-full flex items-center justify-end space-x-4'>
              <Button type='button' variant="destructive" onClick={() => router.push('/dashboard/kelola-soal/deret-angka')}>Batal</Button>
...
export default EditSoalDeretAngkaPage