...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalVerbal.findUnique({
    ...
    try {
        ...
        const existingScoreId = existingScoreTotal?.id_score_verbal;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreVerbal.create({
                data: {
                    benar: correctAnswer,
                salah: wrongAnswer,
                score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes analogi verbal anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreVerbal.update({
            where: {
                id: existingScoreId,
            },
            data: {
