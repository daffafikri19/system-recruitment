...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const existingScoreId = existingScoreTotal?.id_score_sinonim;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreSinonim.create({
                data: {
                    benar: correctAnswer,
                salah: wrongAnswer,
                score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes sinonim anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreSinonim.update({
            where: {
                id: existingScoreId,
            },
            data: {
