...
export const CountDown = ({ waktu_pengerjaan, onCountdownZero }: CountdownProps) => {
    ...
    useEffect(() => {
        if (countdown['w-ketelitian'] <= 20 && startCountdown) {
            setColor(true);
        }
        if (countdown['w-ketelitian'] <= 0 && startCountdown) {
        ...
        if (startCountdown) {
            countdownInterval = setInterval(() => {
                setCountdown((prevTime: any) => {
                    const newTime = {
                        ...prevTime,
                        'w-ketelitian': prevTime['w-ketelitian'] - 1,
                    };
                    return newTime;
                });
            }, 1000);
                ...
                <div>
                    ...
                    {startCountdown ? (
                        <h1 className={cn("text-6xl", color ? "text-red-500" : "")}>
                            {formatTimeCountdown(countdown['w-ketelitian'])}
                        </h1>
                    ) : (
