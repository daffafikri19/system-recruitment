...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalhitungCepat.findUnique({
    ...
    try {
        ...
        const existingScoreId = existingScoreTotal?.id_score_hitung_cepat;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreHitungCepat.create({
                data: {
                    benar: correctAnswer,
                    salah: wrongAnswer,
                    score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes hitung cepat anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreHitungCepat.update({
            where: {
                id: existingScoreId,
            },
            data: {
