...
import { editSoal } from '@/actions/mutations/soal/sinonim/editSoal'
...
const EditSoalSinonimPage = () => {
  const [media, setMedia] = useState<string | null>("");
  const [soal, setSoal] = useState("");
  const router = useRouter();
  const [question, setQuestion] = useState<SoalSinonimProps>({
    id: 0,
    gambar: "",
    soal: "",
    A: "",
        ...
        <CardContent className='py-4'>
          <form action={async formdata => {
            ...
            if (result.status === 200) {
              ...
              return router.push('/dashboard/kelola-soal/sinonim')
            } else {
              toast({
                title: result.message,
                variant: "destructive"
            ...
            <div className='mt-10 w-full flex items-center justify-end space-x-4'>
              <Button disabled={loading} type='button' variant="destructive" onClick={() => router.push('/dashboard/kelola-soal/sinonim')}>Batal</Button>
...
export default EditSoalSinonimPage