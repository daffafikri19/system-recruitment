...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalKalimatTakTeratur.findUnique({
    ...
    try {
        const existingScoreTotal = await prisma.scoreTotalPsikotest.findUnique({
            ...
            select: {
                ...
                id_score_kalimat_tak_teratur: true,
            },
        });
        ...
        const existingScoreId = existingScoreTotal?.id_score_kalimat_tak_teratur;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreKalimatTakTeratur.create({
                data: {
                    benar: correctAnswer,
                    salah: wrongAnswer,
                    score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes kalimat tak teratur anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreKalimatTakTeratur.update({
            where: {
                id: existingScoreId,
            },
            data: {
