...
export async function POST(req: NextRequest) {
    ...
    for (const answer of selectedAnswers) {
        const question = await prisma.soalKemampuanTeknikal.findUnique({
    ...
    try {
        const existingScoreTotal = await prisma.scoreTotalPsikotest.findUnique({
            ...
            select: {
                ...
                id_score_kemampuan_teknikal: true,
            },
        });
        ...
        const existingScoreId = existingScoreTotal?.id_score_kemampuan_teknikal;

        if (!existingScoreId) {
            // If existingScoreId is not found, create a new record
            const createdScore = await prisma.scoreKemampuanTeknikal.create({
                data: {
                    benar: correctAnswer,
                salah: wrongAnswer,
                score: score,
            ...
            return NextResponse.json({
                message: "Berhasil menyimpan jawaban tes kemampuan teknikal anda",
            }, {
                status: 200,
            }
            );
        ...
        await prisma.scoreKemampuanTeknikal.update({
            where: {
                id: existingScoreId,
            },
            data: {
