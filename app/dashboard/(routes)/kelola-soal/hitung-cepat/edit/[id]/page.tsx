...
interface SoalHitungCepatProps {
  id: number;
  gambar: string | null;
  soal: string;
...
const EditSoalHitungCepatPage = () => {
  const [media, setMedia] = useState<string | null>("");
  const [soal, setSoal] = useState("");
  const router = useRouter();
  const { loading, setLoading } = useLoadingContext();

  const [question, setQuestion] = useState<SoalHitungCepatProps>({
    id: 0,
    gambar: "",
    soal: "",
    kunci_jawaban: 0,
...
export default EditSoalHitungCepatPage