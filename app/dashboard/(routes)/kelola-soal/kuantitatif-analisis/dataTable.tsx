...
import { soalKuantitatifAnalisis } from "@prisma/client"
import * as XLSX from 'xlsx';
import { toast } from "@/components/ui/use-toast"
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from "@/components/ui/alert-dialog"
import { addMultipleSoal } from "@/actions/mutations/soal/kuantitatif-analisis/addMultipleSoal"
import { deleteSoal } from "@/actions/mutations/soal/kuantitatif-analisis/deleteSoal"
import { useLoadingContext } from "@/lib/providers/loadingStateProvider"
import { Badge } from "@/components/ui/badge"

interface dataTableProps {
    questionData: soalKuantitatifAnalisis[]
}
...
export const DataTable = ({ questionData }: dataTableProps) => {
    ...
    const [pratinjau, setPratinjau] = useState<soalKuantitatifAnalisis>({
        id: 0,
        gambar: "",
        soal: "",
        A: "",
    ...
    const handleOpenPratinjau = (data: soalKuantitatifAnalisis) => {
        setOpenPratinjau(true);
        setPratinjau(data)
    }

    const [searchQuery, setSearchQuery] = useState("");
    const [filteredData, setFilteredData] = useState<soalKuantitatifAnalisis[]>([]);
    ...
    const handleDownloadTemplate = () => {
        const templateURL = "/file/template-soal-kuantitatif-analisis.xlsx";
    ...
    return (
        <div className="w-full">
            <div className="w-full flex items-center justify-between space-x-8">
                <Button onClick={() => router.push('/dashboard/kelola-soal/kuantitatif-analisis/add')}>Tambah Soal <Plus className="w-4 h-4 ml-2" /></Button>
                                    ...
                                    <TableCell className="text-center">
                                        <div className="flex justify-center space-x-2">
                                            <Button size="icon" variant="outline" asChild>
                                                <Link href={`/dashboard/kelola-soal/kuantitatif-analisis/edit/${question.id}/?id=${question.id}`}>
