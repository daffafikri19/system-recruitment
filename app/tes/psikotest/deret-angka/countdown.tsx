...
export const CountDown = ({ waktu_pengerjaan, onCountdownZero }: CountdownProps) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (params.tes === '1' || !params.tes) {
                ...
                setCountdown({
                    'w-deret-angka': waktu_pengerjaan!,
                });
            }
        }
    }, [waktu_pengerjaan]);
    ...
    useEffect(() => {
        if (countdown['w-deret-angka'] <= 20 && startCountdown) {
            setColor(true);
        }
        if (countdown['w-deret-angka'] <= 0 && startCountdown) {
        ...
        if (startCountdown) {
            countdownInterval = setInterval(() => {
                setCountdown((prevTime: any) => {
                    const newTime = {
                        ...prevTime,
                        'w-deret-angka': prevTime['w-deret-angka'] - 1,
                    };
                    return newTime;
                });
            }, 1000);
                ...
                <div>
                    ...
                    {startCountdown ? (
                        <h1 className={cn("text-6xl", color ? "text-red-500" : "")}>
                            {formatTimeCountdown(countdown['w-deret-angka'])}
                        </h1>
                    ) : (
