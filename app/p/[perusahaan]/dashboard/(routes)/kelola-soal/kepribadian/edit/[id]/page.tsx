...
import { editSoal } from '@/actions/mutations/soal/kepribadian/editSoal'
...
interface SoalKepribadianProps {
  id: number;
  gambar: string | null;
  soal: string;
  A: string;
...
const EditSoalKepribadianPage = () => {
  const [media, setMedia] = useState<string | null>("");
  const [soal, setSoal] = useState("");
  const router = useRouter();
  const { loading, setLoading } = useLoadingContext();

  const [question, setQuestion] = useState<SoalKepribadianProps>({
    id: 0,
    gambar: "",
    soal: "",
    A: "",
        ...
        <CardContent className='py-4'>
          <form action={async formdata => {
            ...
            if (result.status === 200) {
              ...
              setLoading(false)
              return router.push('/dashboard/kelola-soal/kepribadian')
            } else {
              toast({
                title: result.message,
                variant: "destructive"
            ...
            <div className='mt-10 w-full flex items-center justify-end space-x-4'>
              <Button type='button' variant="destructive" onClick={() => router.push('/dashboard/kelola-soal/kepribadian')}>Batal</Button>
...
export default EditSoalKepribadianPage